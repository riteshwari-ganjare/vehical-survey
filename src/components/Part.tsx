...
//     const filteredData = makeFilter ? data.filter(item => item.Make === makeFilter) : data;
//     const vehicleCounts = filteredData.reduce((acc: Record<string, number>, item: EVData) => {
...
//     const makes = Array.from(new Set(data.map(item => item.Make))).map(make => ({ label: make }));
//     const chartData = {
